...
export default function FinancialsPage() {
  ...
  const s = {
    page:    { minHeight: "100vh", background: "#0f172a", color: "#e2e8f0", fontFamily: "system-ui", padding: isMobile ? "0" : "24px" },
    card:    { background: "#1e293b", borderRadius: isMobile ? 0 : 12, padding: isMobile ? "14px" : 20, marginBottom: isMobile ? 8 : 20 },
    th:      { padding: "8px 12px", textAlign: "left" as const, color: "#64748b", borderBottom: "1px solid #334155", fontSize: 12 },
    td:      { padding: isMobile ? "6px 10px" : "8px 12px", borderBottom: "1px solid #0f172a44", fontSize: isMobile ? 11 : 13 },
    section: { fontSize: 14, fontWeight: 700, marginBottom: 14, color: "#94a3b8", letterSpacing: 0.5 },
  };
  ...
  const epsChart = [...income].reverse().map((q: any, i: number, arr: any[]) => {
    const prev = arr[i - 4];
    return { date: q.date?.slice(0, 7), EPS: q.eps, EPS年增: prev ? yoy(q.eps, prev.eps) : null };
  });
  ...
  const grossMargin  = latestIncome.revenue > 0 ? ((latestIncome.grossProfit / latestIncome.revenue) * 100).toFixed(1) + "%" : "N/A";
  ...
  const ttmEps = income.slice(0, 4).reduce((sum: number, q: any) => sum + (q.eps ?? 0), 0);
  const pe = (ttmEps > 0 && quote?.price) ? (quote.price / ttmEps).toFixed(1) : "N/A";
  ...
  return (
    <div style={s.page}>
      ...
      {isMobile ? (
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 11, color: "#64748b" }}>{quote?.name}</span>
          </div>
          {quote && (
            <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
              <span style={{ fontSize: 20, fontWeight: 800 }}>${quote.price?.toFixed(2)}</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: quote.change >= 0 ? "#22c55e" : "#ef4444" }}>
                {quote.change >= 0 ? "+" : ""}{quote.changePercentage?.toFixed(2)}%
              </span>
            </div>
          )}
        </div>
      ...
      <div style={{
        ...
        gap: isMobile ? 6 : 10,
        padding: isMobile ? "12px 12px 6px" : "0",
        marginBottom: isMobile ? 0 : 24,
      }}>
        <MetricCard label="本益比 P/E (TTM)" value={pe} sub="近四季計算" />
        <MetricCard label="毛利率" value={grossMargin} sub={`淨利率 ${netMargin}`} color="#22c55e" />
        <MetricCard label="ROE" value={roe} sub="股東權益報酬" color="#3b82f6" />
        <MetricCard label="負債權益比" value={debtToEquity} sub={`流動比率 ${currentRatio}`} color={parseFloat(debtToEquity) > 2 ? "#ef4444" : "#e2e8f0"} />
        <MetricCard label="FCF Margin" value={fcfMargin} sub="自由現金流率" color="#f59e0b" />
      ...
      {/* ── 季/年切換 ── */}
      <div style={{ display: "flex", gap: 8, padding: isMobile ? "10px 12px 6px" : "0", marginBottom: isMobile ? 0 : 16 }}>
        {(["quarter", "annual"] as Period[]).map(p => (
          <button key={p} onClick={() => setPeriod(p)} style={{
      ...
      {/* ── 內容包裝 ── */}
      <div style={{ padding: isMobile ? "8px 0 0" : "0" }}>
        ...
        {tab === "income" && (
          <>
            <div style={s.card}>
              ...
              {!aiDone && (
                <button onClick={runAI} disabled={aiLoading} style={{
                  width: "100%", background: aiLoading ? "#334155" : "#7c3aed",
            ...
            <div style={s.card}>
              ...
              <ResponsiveContainer width="100%" height={chartH}>
                <BarChart data={incomeChart}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                  <XAxis dataKey="date" tick={{ fill: "#94a3b8", fontSize: isMobile ? 9 : 11 }} />
            ...
            <div style={s.card}>
              ...
              <ResponsiveContainer width="100%" height={isMobile ? 160 : 200}>
                <LineChart data={epsChart}>
                  ...
                  <Tooltip {...tooltipStyle} />
                  <ReferenceLine y={0} stroke="#475569" strokeDasharray="4 4" />
                  <Line type="monotone" dataKey="EPS" stroke="#a78bfa" strokeWidth={2} dot={{ r: 3, fill: "#a78bfa" }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: isMobile ? 11 : 12 }}>
                  <thead>
                    <tr style={{ background: "#0f172a" }}>
                      {(isMobile
                        ? ["日期","營收","YoY","EPS"]
                        : ["日期","營收","YoY","毛利","毛利率","營業利益","淨利","EPS","EBITDA"]
                    ...
                    {income.map((q: any, i: number) => {
                      ...
                      return (
                        <tr key={q.date} style={{ background: i % 2 === 0 ? "transparent" : "#ffffff08" }}>
                          ...
                          <td style={{ ...s.td, fontWeight: 700 }}>${q.eps?.toFixed(2)}</td>
                          {!isMobile && <td style={s.td}>{fmt(q.ebitda)}</td>}
                        </tr>
                      );
                    })}
                  </tbody>
        ...
        {tab === "balance" && (
          <>
            <div style={{
              ...
              <MetricCard label="負債權益比" value={debtToEquity} sub="越低越穩健" color={parseFloat(debtToEquity) > 2 ? "#ef4444" : "#22c55e"} />
              <MetricCard label="總資產" value={fmt(latestBalance.totalAssets)} sub={`總負債 ${fmt(latestBalance.totalLiabilities)}`} />
              <MetricCard label="現金" value={fmt(latestBalance.cashAndCashEquivalents)} sub="手頭現金" color="#3b82f6" />
            </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: isMobile ? 11 : 12 }}>
                  <thead>
                    <tr style={{ background: "#0f172a" }}>
                      {(isMobile
                        ? ["日期","總資產","總負債","現金"]
                        : ["日期","總資產","總負債","股東權益","現金","總債務","流動資產","流動負債"]
                      ).map(h => <th key={h} style={s.th}>{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {balance.map((q: any, i: number) => (
                      <tr key={q.date} style={{ background: i % 2 === 0 ? "transparent" : "#ffffff08" }}>
                        ...
                        <td style={s.td}>{fmt(q.totalAssets)}</td>
                        <td style={s.td}>{fmt(q.totalLiabilities)}</td>
                        {!isMobile && <td style={{ ...s.td, color: "#22c55e" }}>{fmt(q.totalStockholdersEquity)}</td>}
                        <td style={s.td}>{fmt(q.cashAndCashEquivalents)}</td>
                        {!isMobile && <>
                          <td style={{ ...s.td, color: "#ef4444" }}>{fmt(q.totalDebt)}</td>
                          <td style={s.td}>{fmt(q.totalCurrentAssets)}</td>
        ...
        {tab === "cashflow" && (
          <>
            <div style={{
              ...
              <MetricCard label="營業現金流" value={fmt(latestCF.operatingCashFlow)} sub="核心獲利品質" color="#3b82f6" />
              <MetricCard label="自由現金流" value={fmt(latestCF.freeCashFlow)} sub="可分配給股東" color={latestCF.freeCashFlow >= 0 ? "#22c55e" : "#ef4444"} />
              <MetricCard label="資本支出" value={fmt(latestCF.capitalExpenditure)} sub="再投資規模" color="#f59e0b" />
              <MetricCard label="FCF Margin" value={fcfMargin} sub="自由現金流率" />
            </div>
            ...
            <div style={s.card}>
              ...
              <ResponsiveContainer width="100%" height={isMobile ? 220 : 300}>
                <LineChart data={metricsChart}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                  <XAxis dataKey="date" tick={{ fill: "#94a3b8", fontSize: isMobile ? 9 : 11 }} />
                  <YAxis tick={{ fill: "#94a3b8", fontSize: isMobile ? 9 : 11 }} unit="%" />
                  <Tooltip {...tooltipStyle} formatter={(v: any) => v + "%"} />
                  <Legend wrapperStyle={{ fontSize: isMobile ? 11 : 12 }} />
                  <ReferenceLine y={0} stroke="#475569" strokeDasharray="4 4" />
                  <Line type="monotone" dataKey="毛利率"    stroke="#3b82f6" strokeWidth={2} dot={{ r: 2 }} />
                  <Line type="monotone" dataKey="淨利率"    stroke="#10b981" strokeWidth={2} dot={{ r: 2 }} />
                  <Line type="monotone" dataKey="ROE"       stroke="#f59e0b" strokeWidth={2} dot={{ r: 2 }} />
                  <Line type="monotone" dataKey="營業利益率" stroke="#a78bfa" strokeWidth={2} dot={{ r: 2 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: isMobile ? 11 : 12 }}>
                  <thead>
                    <tr style={{ background: "#0f172a" }}>
                      {(isMobile
                        ? ["日期","毛利率","淨利率","EPS"]
                        : ["日期","毛利率","淨利率","營業利益率","ROE","EPS","營收YoY"]
                      ).map(h => <th key={h} style={s.th}>{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {income.map((q: any, i: number) => {
                      ...
                      return (
                        <tr key={q.date} style={{ background: i % 2 === 0 ? "transparent" : "#ffffff08" }}>
                          ...
                          <td style={{ ...s.td, fontWeight: 700 }}>${q.eps?.toFixed(2)}</td>
                          {!isMobile && <td style={s.td}><YoyBadge val={revG} /></td>}
                        </tr>
                      );
                    })}
                  </tbody>
